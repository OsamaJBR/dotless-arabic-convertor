var {replacables, punctuations} = require('../lettersConfig')
...
exports.convertToDotless= async function(char, index, array, text){
    ...
    switch(escape(char)){
        ...
        case '%u0642':
            if(escape(text[index+1]).startsWith('%u06') && !punctuations.includes(escape(text[index+1])) ) array.push(unescape('%u06A1'));
            else array.push(unescape('%u066F'))
            break;
        // Yaa ياء 
        case '%u064A':
            if(escape(text[index+1]).startsWith('%u06') && !punctuations.includes(escape(text[index+1])) ) array.push(unescape('%u066E'));
